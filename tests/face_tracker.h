...
private:
	...
	void track_features(cv::Mat&, Mat&, vector<cv::Point2f>[]);
...
public:
	t_face_tracker();
	~t_face_tracker();
	bool track_face(void);
	std::vector<Rect> detect_and_display_faces(cv::Mat);
	void detect_hand_in_upper_corners(cv::Mat);
