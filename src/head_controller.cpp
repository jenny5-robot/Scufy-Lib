...
bool t_head_controller::setup() {
	...
	head_motors_controller.send_create_motors(2, head_motors_dir_pins, head_motors_step_pins, head_motors_enable_pins);

	jenny5_event create_head_motors_event(MOTORS_CONTROLLER_CREATED_EVENT);
...
void t_head_controller::move_left(unsigned int num_steps) {
	head_motors_controller.send_move_motor(MOVE_X, DIRECTION_LEFT * num_steps);
	head_motors_controller.set_motor_state(MOVE_X, COMMAND_SENT);
...
void t_head_controller::move_right(unsigned int num_steps) {
	head_motors_controller.send_move_motor(MOVE_X, DIRECTION_RIGTH * num_steps);
	head_motors_controller.set_motor_state(MOVE_X, COMMAND_SENT);
...
void t_head_controller::move_up(unsigned int num_steps) {
	head_motors_controller.send_move_motor(MOVE_Y, DIRECTION_UP * num_steps);
	head_motors_controller.set_motor_state(MOVE_Y, COMMAND_SENT);
...
void t_head_controller::move_down(unsigned int num_steps) {
	head_motors_controller.send_move_motor(MOVE_Y, DIRECTION_DOWN * num_steps);
	head_motors_controller.set_motor_state(MOVE_Y, COMMAND_SENT);
